...
SDL_Surface* LoadImage(const char* ImageLocation)
{
	...
	// Checks if there is an error on the location and if there is gives the user another try
	int error;
	do{
		error = 1;
		try{
			...
			printf("WARNING: There is no image at that location.\nPlease enter a valid location.\n");
			scanf("%s", ImageLocation);
		}
	} while (error == 0);
